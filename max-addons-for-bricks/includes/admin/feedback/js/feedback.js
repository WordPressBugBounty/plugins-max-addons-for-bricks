/**
 * Universal Plugin Feedback System JavaScript
 *
...
(function($) {
	...
	class UniversalFeedback {
		 ...
		 */
		initReviewNotices() {
			$(document).on('click', '.dismiss-notice', (e) => {
				e.preventDefault();
				this.dismissReviewNotice($(e.currentTarget));
			});
		}
		 ...
		 * @param {jQuery} button - The dismiss button
		 */
		dismissReviewNotice(button) {
			const notice = button.closest('[data-plugin-slug]');
			const pluginSlug = notice.data('plugin-slug');
			const reason = button.data('reason') || 'dismissed';

			const data = {
				action: `${pluginSlug}_dismiss_review_notice`,
				reason: reason,
				nonce: this.getFeedbackVar(pluginSlug, 'nonce')
			};
			...
			$.post(this.getFeedbackVar(pluginSlug, 'ajax_url'), data)
				...
				.fail(() => {
					// Fallback: hide notice even if AJAX fails
					notice.fadeOut(300);
				});
		}
	...
	window.UniversalPluginFeedback = UniversalFeedback;
	window.FeedbackUtils = Utils;
